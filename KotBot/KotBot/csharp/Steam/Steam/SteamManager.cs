    ...
    public static class SteamManager
    {
        ...
        static void OnChatMsg(SteamFriends.ChatMsgCallback callback)
        {
            if (callback.ChatMsgType == EChatEntryType.ChatMsg)
            {
                ...
                {
                    message = new Message(new Steam.SteamChatClient(callback.ChatRoomID, steamUser.SteamID), new Steam.KSteamUser(callback.ChatterID), callback.Message)
                });
            }
        }
        static void OnChat(SteamFriends.FriendMsgCallback callback)
        {
            if (callback.EntryType == EChatEntryType.ChatMsg)
            {
                ModuleCommunications.MessageRecieved(new MessageArgs()
                {
                    message = new Message(new Steam.SteamPMClient(callback.Sender, steamUser.SteamID), new Steam.KSteamUser(callback.Sender), callback.Message)
                });
            }
